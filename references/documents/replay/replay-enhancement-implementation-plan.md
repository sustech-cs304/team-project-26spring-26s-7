...
目标: 在主链路稳定后再做，避免占用 P0/P1 交付窗口。

| ID | 任务 | 优先级 | 前置条件 | 验收标准 |
| --- | --- | --- | --- | --- |
| R5.1 | 照片主色提取 | P3 | EffectKit API 验证 | 卡片底部背景可随当前照片主色变化，有失败降级 |
| R5.2 | 路线绘制动画 | P3 | MapKit overlay 清理/分段能力验证 | 回放开始时路线逐段绘制，关闭开关后保持现状 |
| R5.3 | Lottie 心情动画 | P3 | 依赖包和 JSON 素材确认 | 根据 mood 显示轻量动画，不阻塞回放 |
| R5.4 | 粒子系统 | P4 | Canvas 性能验证 | 粒子开启后帧率可接受，可关闭 |
| R5.5 | ReplayEndOverlay | P3 | 播放完成事件稳定 | 回放结束显示重新播放/保存/分享入口 |
| R5.6 | RouteSummaryCard 导出 | P4 | 截图、相册写入、权限验证 | 可保存路线总结卡到本地相册 |

实现要点:

1. 导出总结卡涉及 `componentSnapshot`、媒体库写入权限、分享链路，建议单独开任务，不要和视觉改版混在一起。
2. Lottie/粒子都引入素材和性能风险，适合作为演示增强，不应阻塞基础可用版本。
3. 路线绘制动画如果 MapKit 不支持删除/更新局部 polyline，要明确降级为“卡片转场 + marker 波纹”。
