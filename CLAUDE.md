...
3. `./memory/04_test_checklist.md` - 测试清单（**关注通过率**）
...
**规则**: 每当一个测试用例通过，必须标记 "✅ 通过" 并更新测试清单。

### 测试状态更新流程

```
测试执行 → 通过 → 更新 memory/04_test_checklist.md
                    ├── 修改对应测试用例状态为 ✅
                    ├── 记录执行日期
                    └── 在"测试执行记录"区域添加条目
```
...
当用户报告测试结果时，AI 必须：
1. **立即确认**: "收到，测试通过，正在更新清单..."
2. **更新清单**: 修改 `04_test_checklist.md` 中对应条目
3. **汇报进度**: 告知当前通过率（如：数据层单元测试 5/12）

### 通过率追踪

| 测试类型 | 目标 | 当前 |
|---------|------|------|
| 数据层单元测试 | 12/12 | 查看 `04_test_checklist.md` |
| 工具类单元测试 | 6/6 | 查看 `04_test_checklist.md` |
| UI 自动化测试 | 10/10 | 查看 `04_test_checklist.md` |
| 性能测试 | 4/4 | 查看 `04_test_checklist.md` |
